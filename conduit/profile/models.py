...
followers_assoc = db.Table("t_followers_assoc",
    db.Column("follower",db.Integer,db.ForeignKey("t_userprofile.user_id")),
    db.Column("followed_by",db.Integer,db.ForeignKey("t_userprofile.user_id"))
    )
...
class UserProfile(Model,SurrogatePK):
    ...
    user_id = reference_col("t_user",nullable=False)
    ...
    def is_following(self,profile):
        return bool(self.follows.filter(followers_assoc.c.followed_by ==
        profile.id).count())
